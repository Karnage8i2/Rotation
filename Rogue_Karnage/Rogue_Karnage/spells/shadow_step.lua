...
local function logics(entity_list, target_selector_data, best_target, closest_target)
    ...
    if not is_logic_allowed then
        if console and type(console.print) == "function" then
            console.print("[ShadowStep] Blocked: is_logic_allowed=false")
        end
        return false
    end
    ...
    if menu_elements_shadow_step_base.cast_mode:get() == 0 then -- Always mode
        if best_target then
            ...
            if target_dist > (min_range * min_range) and target_dist <= (spell_range * spell_range) then
                if cast_spell.target(best_target, spell_id_shadow_step, 0.5, false) then
                    ...
                    if console and type(console.print) == "function" then
                        console.print("[ShadowStep] cast_spell.target failed in Always mode")
                    end
                end
            end
        else
            if console and type(console.print) == "function" then
                console.print("[ShadowStep] No best_target in Always mode")
            end
        end
    else -- Gap Close mode
        ...
        local step_target = furthest_target or best_target
        if step_target then
            local target_dist = player_position:squared_dist_to_ignore_z(step_target:get_position())
            if target_dist > (min_range * min_range) and target_dist <= (spell_range * spell_range) then
                if cast_spell.target(step_target, spell_id_shadow_step, 0.5, false) then
                    local current_time = get_time_since_inject()
                    next_time_allowed_cast = current_time + menu_elements_shadow_step_base.cast_delay:get()
                    last_cast_position = step_target:get_position()
                    last_target_id = step_target:get_id()
                    console.print("Rouge Plugin, Casted Shadow Step Gap Close")
                    return true
                else
                    if console and type(console.print) == "function" then
                        console.print("[ShadowStep] cast_spell.target failed in Gap Close mode")
                    end
                end
            else
                if console and type(console.print) == "function" then
                    console.print("[ShadowStep] step_target out of range: dist_sqr=" .. tostring(target_dist) .. ", min=" .. tostring(min_range) .. ", max=" .. tostring(spell_range))
                end
            end
        else
            if console and type(console.print) == "function" then
                console.print("[ShadowStep] No step_target in Gap Close mode")
            end
        end
    end
