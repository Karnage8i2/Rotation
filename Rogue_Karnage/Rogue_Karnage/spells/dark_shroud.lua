...
local menu_elements_dark_shroud_base = 
{
    ...
    hp_usage_shield       = slider_float:new(0.0, 1.0, 1.00, get_hash(my_utility.plugin_label .. "%_in_which_shield_will_cast_base_shroud_2"))
}
...
local function logics()
    ...
    -- In Helltide/NMD mode, treat Dark Shroud more proactively by raising the
    -- effective minimum HP threshold so the shield is applied earlier.
    local effective_min_percentage = menu_min_percentage
    do
        ...
        if helltide_nmd_enabled and effective_min_percentage < 0.80 then
            effective_min_percentage = 0.80
        end
    end

    if health_percentage > effective_min_percentage then
    ...
    if not has_shroud then
        console.print("has shroud")
        return false
    end
    ...
    if cast_spell.self(spell_id_dark_shroud, 0.0) then
        ...
        console.print("Rogue Plugin, Casted Dark Shroud");
        return true;
    end;
