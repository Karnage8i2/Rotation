...
local menu_elements_heartseeker_base = 
{
    ...
    filler_range       = slider_float:new(5.0, 30.0, 20.0, get_hash(my_utility.plugin_label .. "heartseeker_filler_range")),
}
...
local function menu()
    ...
    if menu_elements_heartseeker_base.tree_tab_bone:push("Heartseeker") then
        ...
        menu_elements_heartseeker_base.filler_range:render("Filler Range", "Max distance for Heartseeker filler casts", 1)
...
local function logics(target)
    ...
    -- Prioritize good Heartseeker casts when using higher cast delays:
    -- Always allow casts on priority targets (elite/champion/boss), but
    -- require at least 2 hits for normal trash to avoid wasting slow casts.
    local is_priority_target = false
    if best_target.is_boss and best_target:is_boss() then
        is_priority_target = true
    elseif best_target.is_champion and best_target:is_champion() then
        is_priority_target = true
    elseif best_target.is_elite and best_target:is_elite() then
        is_priority_target = true
    end

    if not is_priority_target and best_cast_hits < 2 then
        return false
    end
