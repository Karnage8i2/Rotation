    ...
    if is_filter_enabled then
        if distance_sqr > (spell_range * spell_range) then
            if menu_elements_puncture_base.close_distance:get() then
                -- Respect global Disable Auto Movement toggle to avoid unwanted pathing
                local disable_auto_movement = false
                pcall(function()
                    if menu and menu.menu_elements and menu.menu_elements.disable_auto_movement then
                        disable_auto_movement = menu.menu_elements.disable_auto_movement:get()
                    end
                end)

                if not disable_auto_movement then
                    pathfinder.request_move(target_position)
                    console.print("Moving closer to target")
                    return true
                else
                    console.print("Auto movement disabled - not moving closer for Puncture")
                    return false
                end
            else
                console.print("Skip")
