...
local menu_elements =
{
    ...
    min_hits              = slider_int:new(1, 20, 1, get_hash(my_utility.plugin_label .. "smoke_grenade_base_min_hits_to_casttrap_base_pos")),
    ...
    spell_range   = slider_float:new(1.0, 12.0, 10.00, get_hash(my_utility.plugin_label .. "smoke_grenade_spell_range")),
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    -- High-value targets and normal packs handling
    local high_value_present = (elite_units_count or 0) > 0 or (champion_units_count or 0) > 0 or (boss_units_count or 0) > 0
    -- Treat packs of 5+ normals as valid AoE even without elites/bosses
    local normal_pack_ok = (not high_value_present) and (all_units_count or 0) >= 5

    -- Skip lone normal enemies when no high-value targets and no keybind override
    if not keybind_can_skip and not high_value_present and (all_units_count or 0) <= 1 then
        return false
    end
    
    -- Skip if not enough enemies and no special units/pack and not using keybind override
    if not (keybind_can_skip or high_value_present or normal_pack_ok) and all_units_count < effective_min_enemies then
        return false
    end
    ...
    if not (constains_relevant or normal_pack_ok) and area_data.score < menu_elements.soft_score:get() and not keybind_can_skip  then
        return false;
    end
