...
local function logics(entity_list, target_selector_data, target)
    ...
    -- High-value targets (elites/champions/bosses) and normal packs handling
    ...
    -- Skip lone normal enemies when there are no high-value targets and no keybind override
    if not keybind_can_skip and not high_value_present and (all_units_count or 0) <= 1 then
        if debug_console then
            console.print("poison_trap: Skipping cast on single normal enemy")
        end
        return false
    end

    -- Skip if not enough enemies and not using keybind override and no boss/pack/high-value present
    ...
    if not (keybind_can_skip or boss_present or high_value_present or normal_pack_ok) and all_units_count < effective_min_enemies then
        if debug_console then
    ...
    -- Skip if no relevant targets and score is too low, but allow good normal packs
    if not (contains_relevant or normal_pack_ok or boss_present) and area_data.score < menu_elements.soft_score:get() and not keybind_can_skip then
        if debug_console then
